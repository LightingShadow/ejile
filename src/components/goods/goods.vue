<template>
  <div class='goods'>
    ...
    <shopcart
     ref='shopcart' :select-foods='selectFoods' :delivery-price='seller.deliveryPrice' :min-price='seller.minPrice'></shopcart>
     <food ref='food' @add='getFood' :food='selectedFood'></food>
  </div>
</template>
...
<script>
  ...
  created(){
    ...
    this.$http.get('http://localhost:3000/api/goods').then((response)=>{
      let data=response.body.goods;
